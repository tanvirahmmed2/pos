...
const AddSupplierForm = () => {
    const { fetchSupplier, setIsSupplierBox } = useContext(Context)
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            fetchSupplier()
            if (setIsSupplierBox) setIsSupplierBox(false)
        } catch (error) {
            console.log(error)
    ...
    return (
        <form onSubmit={handleSubmit} className='flex flex-col w-full bg-white'>
            ...
            <div className='flex items-center justify-between p-6 border-b border-slate-100'>
                <h2 className='text-lg font-bold text-slate-800 tracking-tight'>Add New Supplier</h2>
                <button 
                    type='button' 
                    onClick={() => setIsSupplierBox && setIsSupplierBox(false)} 
