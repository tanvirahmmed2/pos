    ...
    <div className='w-full max-w-7xl mx-auto p-4 md:p-6 flex flex-col gap-6 bg-slate-50 min-h-screen'>
      ...
      <div className='flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-white p-6 rounded-2xl shadow-sm border border-slate-100'>
        ...
        <div className='bg-slate-50 px-4 py-2 rounded-xl border border-slate-100'>
          ...
          <span className='text-slate-900 text-xs uppercase tracking-wider ml-2 font-bold'>Ready to Ship</span>
        </div>
      </div>
      ...
      <div className='w-full flex flex-col gap-4'>
        {loading ? (
          <p className='text-center text-slate-400 animate-pulse font-bold py-20'>Loading Confirmed Orders...</p>
        ) : orders.length === 0 ? (
            ...
            <div className='md:col-span-2 flex md:flex-col items-center justify-center gap-2'>
              {confirmShip === order.order_id ? (
                <div className='flex flex-col gap-2 w-full'>
                  ...
                  <button onClick={() => shipOrder(order.order_id)} className='w-full bg-slate-800 hover:bg-slate-900 text-white p-2.5 rounded-xl flex items-center justify-center gap-1 text-xs font-bold'>
                ...
                <div className='flex flex-col gap-2 w-full'>
                  ...
                  <button onClick={() => cancelOrder(order.order_id)} className='w-full bg-slate-800 hover:bg-slate-900 text-white p-2.5 rounded-xl flex items-center justify-center gap-1 text-xs font-bold'>
                    <FaCheck /> Yes, Cancel
                  </button>
                ...
                <div className='w-full flex flex-col gap-2'>
                  <button onClick={() => setConfirmShip(order.order_id)} className='w-full bg-slate-800 hover:bg-slate-900 text-white p-2.5 rounded-xl flex items-center justify-center gap-2 font-bold text-sm transition-colors'>
                    <MdOutlineLocalShipping size={18} /> Ship
                  </button>
                  <button onClick={() => setConfirmCancel(order.order_id)} className='w-full bg-slate-50 text-slate-800 hover:bg-slate-100 p-2.5 rounded-xl flex items-center justify-center gap-2 font-bold text-sm transition-colors'>
