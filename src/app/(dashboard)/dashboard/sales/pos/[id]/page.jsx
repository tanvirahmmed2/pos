...
const POSSLIPPAGE = ({ params }) => {
    ...
    if (loading) return (
        <div className="fixed inset-0 flex items-center justify-center bg-slate-50">
            <p className="text-xs uppercase tracking-widest text-slate-400 animate-pulse">Loading slip…</p>
        </div>
    )
    ...
    if (!order) return (
        <div className="fixed inset-0 flex items-center justify-center bg-slate-50">
            <div className="text-center">
                <p className="text-xs uppercase tracking-widest text-slate-400">Slip not found</p>
        ...
        <div className="min-h-screen w-full flex flex-col items-center py-6 sm:py-12 px-4 print:p-0 print:bg-white overflow-x-hidden">
            ...
            <div className="w-full max-w-150 bg-white shadow-[0_20px_50px_rgba(0,0,0,0.1)] p-5 sm:p-7 border border-slate-100 flex flex-col gap-4 print:shadow-none print:border-none print:w-full">
                ...
                <header className="text-center space-y-1">
                    ...
                    <p className="text-[10px] text-slate-500">{siteData?.address || "Pakuritala Bazar, Tarakanda"}</p>
                        ...
                        {order.items?.map((item, index) => (
                            <div key={index} className="grid grid-cols-12 gap-1 min-w-0">
                                <div className="col-span-7 min-w-0">
                                    <p className="text-[14px] font-bold text-slate-900 leading-none truncate block">{item.name}</p>
                                    <p className="text-[13px] text-slate-400 font-mono mt-1">@ {Number(item.price).toFixed(2)}</p>
                                </div>
                                <span className="col-span-2 text-[13px] text-center pt-0.5 font-mono text-slate-600">x{item.quantity}</span>
                                <span className="col-span-3 text-[14px] text-right pt-0.5 font-bold font-mono text-slate-900">
                ...
                <div className="mt-4 border-t-2 border-black pt-3 space-y-2">
                    ...
                    {order.total_discount_amount > 0 && (
                        <div className="flex justify-between text-xs text-slate-900 italic">
                            <span>Discount</span>
                ...
                <footer className="mt-6 text-center">
                    ...
                    <p className="mt-6 text-[8px] uppercase tracking-widest text-slate-400">
                        © {new Date().getFullYear()} {siteData?.website_name || "Store Name"}
                    </p>
                </footer>
                ...
                <div className="mt-6 flex flex-col gap-2 print:hidden">
                    <button 
                        ...
                        className="w-full bg-black text-white cursor-pointer py-3 text-[10px] font-bold uppercase tracking-widest rounded active:bg-slate-800 transition-colors shadow-lg"
                    >
                        Print Receipt
                    </button>
                    <Link 
                        href="/dashboard/sales/pos" 
                        className="w-full text-center py-3 text-[10px] font-bold uppercase tracking-widest border border-slate-200 rounded text-slate-500 hover:bg-slate-50"
                    >
                        Return to POS
                    </Link>
                </div>
